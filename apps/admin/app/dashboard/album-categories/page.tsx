...
export default function AlbumCategoriesPage() {
  ...
  function startEdit(category: AlbumCategory) {
    ...
    try {
      ...
      if (editingId) {
        await withAuthRefresh(
          () => updateAlbumCategory(editingId, input),
          redirectToLogin,
        );
      } else {
        await withAuthRefresh(
          () => createAlbumCategory(input),
          redirectToLogin,
        );
      }
  ...
  async function handleDelete(category: AlbumCategory) {
    const confirmed = window.confirm(
      `Delete category "${category.name}"? Albums using it will block deletion.`,
    );
    ...
    try {
      await withAuthRefresh(
        () => deleteAlbumCategory(category.id),
        redirectToLogin,
      );
      await loadCategories();
    } catch (caughtError) {
